...
class MiddlewareRegistry {
   ...
   * @param meta An object of meta data { name, before, after }
   * @param factory The middleware (decorator) to apply to the service
   * @param contextList An array of hierarchical context, ['Universe', 'Earth', 'NZ']
   */
  ...
  getFactory(context = GLOBAL_CONTEXT) {
    ...
    if (!this._factoryCache[context]) {
      ...
      const factories = matches.map(m => m.factory);
      const names = matches.map(m => m.displayName || m.name);
      const factory = compose(...factories)(this.service);
